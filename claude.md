...
| Toucher à la vue d'activité de la baie (LEDs live) | network_diag.py / app.py | section « Vue d'activité baie » : thread démon `_activite_loop` (poll tant qu'un navigateur bat, se rendort sinon, **cadence adaptative** sur switch lent mais **courte sur les 3 premiers cycles** `_activite_rechauffe` — 2 relevés = 1er débit, le bandeau montre « démarrage du relevé… » entre-temps), `_noms_interfaces` (nom/type/vitesse, caché ~90 s, refresh async via `_maj_noms_interfaces`), `_poll_switch_ports` (compteurs par cycle via `app._snmp_bulk_cols` = **GETBULK v2c multi-colonnes**, repli GETNEXT), `_mapping_baie_ifindex` (**manuel > topologie > nom d'interface `_port_physique_depuis_nom` > repli naïf opt-in** — le n° de port ≠ ifIndex !), `_etat_led` (clignote sur les paquets, `stale` après 3 manques, **plafond de plausibilité débit/pps** — un compteur qui dépègue/boucle/bascule 64↔32 injectait des Mpps fantômes, **anti-rebond `_ACTIVITE_DEBOUNCE`=2 sur idle↔traffic** + seuil `pps_mini` par défaut 15 : comptes de « ports actifs » stables et identiques entre instances), `_poll_poe` (POWER-ETHERNET-MIB `pethPsePortTable` indexée `groupe.port` = **n° de port physique, pas ifIndex**), `calibrer_port_baie` ; routes `GET /api/baie/activite` + `POST /api/baie/activite/calibrer` (can_write) ; colonne `baie_slot_ports.if_index` ; `templates/baie_brassage.html` (`#sel-activite`, `.port-act-*`) ; widget dashboard `network-activity` ; **compteurs live rien en base** ; clés `diag_baie_activite_seuil_err`/`_pps_mini`/`_repli_naif` |
| Toucher au moniteur réseau de la baie (modale) | network_diag.py / app.py | bouton « 📊 Moniteur » sur `/baie` → `templates/baie_brassage.html` (`#moniteur-modal`, objet JS `MoniteurModal`) ; `_activite_journal` (deque, dédoublonné via `_journal()`) + `_activite_detail` (compteurs bruts par port) ; route `GET /api/baie/activite/moniteur` (lecture) ; **assistant « calibrer par débranchement »** : `assistant_calibration`/`_maj_assistant_calibration` (fenêtre `_CALIB_FENETRE` 90 s, détecte l'ifIndex qui a changé d'`oper`), route `POST /api/baie/activite/calibrer/assistant` (can_write, action start/stop) ; onglet capture : `capturer_trafic`/`lancer_capture_baie`/`statut_capture_baie` (thread démon, réutilise `etat_capture`/palier 2) ; route `POST /api/baie/activite/capture` (**can_write**, comme `/api/diag-reseau/snapshot` — lancer une capture consomme le réseau/système hôte) ; clé `diag_baie_capture_duree_s` |
...
**Dernier update** : 2026-09-02 (v2.19.12 — Vue d'activité baie : comptes de « ports actifs » stables et cohérents entre instances — seuil `diag_baie_activite_pps_mini` 1→15 (le bruit de fond L2 marquait tout port actif) + anti-rebond `_ACTIVITE_DEBOUNCE`=2 sur idle↔traffic, décision d'état sur les valeurs instantanées)
