...
| Toucher à la vue d'activité de la baie (LEDs live) | network_diag.py / app.py | section « Vue d'activité baie » : thread démon `_activite_loop` (poll SNMP tant qu'un navigateur bat, se rendort sinon), `_iftable_switch` (cache 60 s) + `_poll_ports_cibles` (GET SNMP ciblé sur les ports mappés), `_mapping_baie_ifindex` (topologie FDB + repli `numero==ifIndex`, renvoie aussi la source par port), `_etat_led` (clignote sur les paquets, état `stale` après 3 relevés manqués) ; route `GET /api/baie/activite` ; `templates/baie_brassage.html` (`#sel-activite`, `.port-act-*`) ; widget dashboard `network-activity` (`client_dashboard.html` + `base.html`) ; **rien en base** (in-memory), clés `diag_baie_activite_seuil_err`/`diag_baie_activite_pps_mini` |
...
**Dernier update** : 2026-09-01 (v2.19.8 — Moniteur réseau de la baie (modale Ports/Journal/Capture) + LEDs fiabilisées : relevé SNMP ciblé, clignotement sur paquets, état « stale »)
