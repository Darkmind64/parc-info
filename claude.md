...
**Dernier update** : 2026-09-02 (v2.19.10 — Vue d'activité baie : PoE par port (`_poll_poe`), assistant « calibrer par débranchement », plafond de plausibilité débit/pps (compteurs SNMP défectueux) ; `interroger_equipement` + `_snmp_get_typed` (UPS) consolidés sur l'attribution par OID / GETBULK)
