...
| Toucher au SNMP (scan + diagnostic) | app.py | `_snmp_get` (GET OCTET STRING), `_snmp_get_typed` (GET typé), `_snmp_walk` (GETNEXT), `_snmp_bulk_cols` (GETBULK v2c multi-colonnes, attribution par préfixe d'OID) + `_ber_decoder_*` ; tous vérifient le request-id de la réponse (switch lent = réponse tardive) ; OIDs `_OID_IF_*` / `_OID_DOT3_*` dans network_diag.py ; encodage BER fait main, aucune dépendance |
...
**Dernier update** : 2026-09-01 (v2.19.9 — Vue d'activité baie : mapping port↔interface par nom d'interface + calibration manuelle ; SNMP fiabilisé (GETBULK `_snmp_bulk_cols`, vérif request-id, cadence adaptative))
