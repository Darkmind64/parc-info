...
| Toucher à la vue d'activité de la baie (LEDs live) | network_diag.py / app.py | section « Vue d'activité baie » : thread démon `_activite_loop` (poll tant qu'un navigateur bat, se rendort sinon, **cadence adaptative** sur switch lent mais **courte sur les 3 premiers cycles** `_activite_rechauffe` **et tant qu'un assistant de calibration attend**), purge par-client de `_activite_prev`/`_switch_ok`/`_etat_mappe`/`_calib` ; `_noms_interfaces` (caché ~90 s, refresh async `_maj_noms_interfaces` avec **back-off `_ACTIVITE_NOMS_RETRY` sur échec**, flag `maj_en_cours` sous `_activite_lock`), `_poll_switch_ports` (**GETBULK v2c multi-colonnes** `app._snmp_bulk_cols`, repli GETNEXT ; `_activite_hc` passe à `False` seulement après `_ACTIVITE_NEG_CONFIRME`=2 négatifs, re-test tous les `_ACTIVITE_REPROBE_CYCLES`=50), `_mapping_baie_ifindex` (**manuel > topologie > nom d'interface `_port_physique_depuis_nom` > repli naïf opt-in** — le n° de port ≠ ifIndex !), `_etat_led` (clignote sur les paquets, `stale` après 3 manques, **plafond de plausibilité débit/pps**, **anti-rebond `_ACTIVITE_DEBOUNCE`=2** + seuil `pps_mini` défaut 15), `_poll_poe` (POWER-ETHERNET-MIB `pethPsePortTable` indexée `groupe.port` = **n° de port physique** ; même latch 2-négatifs que `_activite_hc`), `calibrer_port_baie` (**refuse un n° de port hors du slot**) ; routes `GET /api/baie/activite` + `POST /api/baie/activite/calibrer` (can_write) ; colonne `baie_slot_ports.if_index` ; `templates/baie_brassage.html` (`#sel-activite`, `.port-act-*`, poll suspendu sur `document.hidden`) ; widget dashboard `network-activity` ; **compteurs live rien en base** ; clés `diag_baie_activite_seuil_err`/`_pps_mini`/`_repli_naif` |
...
**Dernier update** : 2026-09-02 (v2.19.13 — Moniteur réseau baie : audit de fiabilité (11 correctifs) — capacités SNMP (compteurs 64 bits / PoE) non condamnées sur un seul relevé raté + re-test périodique ; `_activite_calib` sous verrou ; écriture de calibration hors du GET ; ménage mémoire par client parti ; assistant de calibration durci (cadence courte forcée, décision au calme, interface rebranchée en dernier) ; `calibrer_port_baie` refuse un port hors slot ; menu déroulant qui ne se referme plus au refresh ; relevé suspendu sur onglet caché)
